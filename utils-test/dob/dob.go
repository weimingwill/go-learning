...
func ageFromDateOfBirth(dob string) (int, int) {
	...
	ageMonth := now.Month() - dobTime.Month()
	ageDay := now.Day() - dobTime.Day()

	if ageMonth < 0 || (ageMonth == 0 && ageDay < 0) {
		age = age - 1
	}

	// fmt.Println("month", monthToInt(now.Month().String()))
	// dobMonth :=
	// nowMonth :=
	...
	// fmt.Println("dobDayMonth", dobDayMonth)
	// fmt.Println("nowDayMonth", nowDayMonth)
